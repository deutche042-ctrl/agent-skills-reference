    ...
    *,
    ...
    for run in runs:
        ...
        while idx < len(run):
            ...
            if remaining < 2:
                break
            if remaining >= primary_per_tile:
                cols, rows = primary_cols, primary_rows
                chunk_size = primary_per_tile
            else:
                cols, rows = pick_grid(
                    med_aspect, remaining, max_per_tile=max_per_tile,
                )
                chunk_size = min(cols * rows, remaining)
            chunk_indices = run[idx: idx + chunk_size]
            idx += chunk_size
