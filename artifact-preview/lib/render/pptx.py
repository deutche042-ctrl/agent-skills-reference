...
def find_libreoffice_binary() -> str | None:
    ...
    Tested binaries (in order): ``libreoffice``, ``soffice``. Both are
    valid headless converters; ``soffice`` is the macOS-bundled name
    inside ``LibreOffice.app/Contents/MacOS/``.
    """
