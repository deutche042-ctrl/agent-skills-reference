"""
probe_excel.py
Excel probing script: read all sheet names and, for each sheet, output the first 15 rows as a raw matrix.
Optimized for data-processing scenarios: focus on information such as data types, missing values, and merged cells.
Requirements: do not omit blank rows; identify and output merged-cell ranges.
"""
import json
import sys
...
# Summary-row keywords used to flag likely total/subtotal rows.
...
def analyze_column(ws, col_idx):
    """Analyze the characteristics of a single column"""
...
def build_profile(path):
    ...
    for name in wb.sheetnames:
        ...
        result["sheets"].append({
            ...
            "special_rows": detect_special_rows(ws),
