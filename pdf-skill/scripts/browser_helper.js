...
function loadPlaywright() {
  try {
    ...
    // npx cache + common global dirs: Playwright is frequently only present under
    // ~/.npm/_npx/<hash>/node_modules (e.g. a prior `npx playwright ...` run, or when
    // permissions stop `npm install` from writing into the skill's own
    // scripts/node_modules). Probe these so the script self-locates Playwright instead
    // of forcing the caller to discover the path and set NODE_PATH by hand.
    const homeCandidates = new Set();
    [os.homedir(), process.env.HOME, '/root', '/home/user'].filter(Boolean)
      .forEach(h => homeCandidates.add(h));

    for (const home of homeCandidates) {
      const npxBase = path.join(home, '.npm', '_npx');
      let entries = [];
      try {
        entries = fs.readdirSync(npxBase);
      } catch (_) {
        continue; // no npx cache under this home
      }
      entries.forEach(entry => searchRoots.add(path.join(npxBase, entry, 'node_modules')));
    }

    ['/usr/local/lib/node_modules', '/usr/lib/node_modules'].forEach(p => searchRoots.add(p));

    for (const base of searchRoots) {
      if (!base) continue;
      // Accept either the full `playwright` package or `playwright-core` (both expose
      // a `chromium` browser type, which is all html_to_pdf.js needs).
      for (const modName of ['playwright', 'playwright-core']) {
        const pkgPath = path.join(base, modName, 'package.json');
        if (!fs.existsSync(pkgPath)) continue;
        try {
          const requireFromBase = Module.createRequire(pkgPath);
          return requireFromBase(modName);
        } catch (_) {
          // try next candidate
        }
      }
    ...
    throw new Error(
      'Playwright module not found. Install it (npm install -g playwright, or in the ' +
      'skill scripts/ dir), set PLAYWRIGHT_PATH / NODE_PATH to its location, then run ' +
...
function getBrowserCacheDirectories() {
  ...
  if (process.platform !== 'win32') {
    ...
    // Explicitly add the cache directories of common sandbox/container users
...
function collectSystemBrowsers() {
  ...
  // Check common system paths directly (without relying on command -v)
