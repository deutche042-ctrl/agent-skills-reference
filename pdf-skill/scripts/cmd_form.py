...
def form_fill(pdf_path: str, output_path: str, data: dict):
    ...
    for field_id, value in data.items():
        ...
        # Validate radio values (options are export states, e.g. "/Choice1")
        if field_type == "radio" and "options" in field_info:
            valid_states = field_info["options"]
            if value not in valid_states and f"/{value}" not in valid_states:
                errors.append(f"Invalid value for field {field_id}, options: {valid_states}")
    ...
    def fill_field(field, parent_name=""):
        ...
        if full_name in data:
            ...
            if field_type == "checkbox":
                ...
                # Radio: /V on the parent must be the chosen widget's on-state (a Name),
                # and each kid widget's /AS must be set to that state or /Off. Setting a
                # String here (the old behavior) never selects the button.
                name_val = str(value) if str(value).startswith("/") else f"/{value}"
                field["/V"] = pikepdf.Name(name_val)
                kids = field.get("/Kids")
                if kids:
                    for kid in kids:
                        ap = kid.get("/AP")
                        kid_states = [str(k) for k in ap["/N"].keys()] if (ap and "/N" in ap) else []
                        if name_val in kid_states:
                            kid["/AS"] = pikepdf.Name(name_val)
                        else:
                ...
                # Text, dropdown, listbox, and other value-based fields
