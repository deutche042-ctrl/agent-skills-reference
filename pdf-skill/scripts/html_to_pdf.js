...
async function convertHTMLToPDF(inputFile, outputFile, customCSS) {
  ...
  try {
    ...
    if (errorMsg.includes('shared libraries') || errorMsg.includes('.so')) {
      ...
      console.error('');
      console.error('On Ubuntu/Debian, prefer:');
      console.error('  npx playwright install-deps chromium');
      console.error('');
      console.error('If system libraries are still missing, ask the environment administrator to install the');
      console.error('Chromium/Playwright runtime packages for this OS (do not run privileged apt from agent code).');
      console.error('Typical package names include: libgbm1 libasound2 libatk-bridge2.0-0 libdrm2 libxkbcommon0');
      console.error('libxcomposite1 libxdamage1 libxfixes3 libxrandr2 libatspi2.0-0 libcups2');
      console.error('');
      process.exit(1);
    }
  ...
  try {
    ...
    if (hasMermaid) {
      // Fail loudly if the Mermaid library itself never loaded (e.g. CDN blocked),
      // rather than silently exporting a PDF containing raw diagram source.
      const mermaidLibLoaded = await page.evaluate(() => typeof window.mermaid !== 'undefined');
      if (!mermaidLibLoaded) {
        console.error('\n❌ Mermaid diagrams are present but the Mermaid library failed to load.');
        console.error('   Cause: the Mermaid CDN (cdn.jsdelivr.net) is unreachable in this environment.');
        console.error('   Fix: pre-render the diagram to a static image/SVG and embed it with <img>,');
        console.error('        or vendor mermaid.min.js locally. Otherwise the PDF shows raw diagram source.');
        await browser.close();
        process.exit(1);
      }
      ...
      try {
        ...
        console.error('\n❌ Mermaid library loaded but the diagram did not render within 30s.');
        console.error('   Check the diagram syntax; the PDF may show raw diagram source instead of an SVG.');
        await browser.close();
        process.exit(1);
      }
    ...
    const katexStatus = await page.evaluate(() => {
      ...
      // "Requested" = the HTML actually references KaTeX. Gating the loud failure on this
      // (rather than on a "$" appearing in the text) avoids false positives from a stray
      // dollar sign in normal prose when no math was intended.
      const katexRequested = !!document.querySelector('script[src*="katex" i], link[href*="katex" i]');
      return { hasKatexLib, hasRenderedKatex, hasUnrenderedMath, katexRequested };
    });

    // Fail loudly if KaTeX was requested but the library never loaded (e.g. CDN blocked).
    if (katexStatus.katexRequested && !katexStatus.hasKatexLib) {
      console.error('\n❌ KaTeX was included but the library failed to load.');
      console.error('   Cause: the KaTeX CDN (cdn.jsdelivr.net) is unreachable in this environment.');
      console.error('   Fix: pre-render formulas to images and embed them with <img>, or vendor');
      console.error('        katex.min.js/.css locally. Otherwise the PDF shows raw "$...$" source.');
      await browser.close();
      process.exit(1);
    }
