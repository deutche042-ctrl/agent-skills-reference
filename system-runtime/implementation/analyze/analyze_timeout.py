    ...
    def process(stmt_list: List[ast.stmt], lenv: Dict[str, object]):
        for s in stmt_list:
            ...
            for node in ast.walk(s):
                if isinstance(node, ast.Call):
                    ...
                    if isinstance(node.func, ast.Name) and node.func.id == "input":
                        ...
                        logger.debug(f"Detected input call at line {node.lineno}")
                    if isinstance(node.func, ast.Attribute) and node.func.attr == "input":
                        definite_input_calls.append(node)
                        logger.debug(f"Detected input attribute call at line {node.lineno}")

            # if isinstance(s, ast.Assign) and isinstance(s.value, ast.Call):
            #     call = s.value
            #     # 检测赋值语句中的 input() 调用
            #     if isinstance(call.func, ast.Name) and call.func.id == "input":
            #         logger.debug(f"[INPUT_DETECT] Found input() in assignment at line {call.lineno}, col {call.col_offset}")
            #         definite_input_calls.append(call)
            #     if isinstance(call.func, ast.Attribute) and call.func.attr == "input":
            #         logger.debug(f"[INPUT_DETECT] Found attribute input call in assignment at line {call.lineno}")
            #         definite_input_calls.append(call)
            ...
            if isinstance(s, ast.Expr) and isinstance(s.value, ast.Call):
                ...
                # input 检测
                if isinstance(call.func, ast.Name) and call.func.id == "input":
                    logger.debug(f"[INPUT_DETECT] Found input() call at line {call.lineno}, col {call.col_offset}")
                    definite_input_calls.append(call)
                if isinstance(call.func, ast.Attribute) and call.func.attr == "input":
                    logger.debug(
...
def static_analyze_code(code: str) -> Optional[CodeAnalyzeResult]:
    logger.debug(f"enter static_analyze_code.")

    try:
        tree = ast.parse(code)
        logger.debug(f"[AST_DEBUG] Top-level nodes: {[type(n).__name__ for n in tree.body]}")
        ...
        if isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.value, ast.Call) and isinstance(
            ...
            if any(isinstance(n, ast.ClassDef) and n.name == cls_name for n in ast.walk(tree)):
                ...
                if isinstance(tgt, ast.Name):
                    ...
                    logger.debug(f"[INSTANCE_COLLECT] Collected instance: {tgt.id} -> {cls_name}")

    func_defs = get_function_defs(tree)
    logger.debug(f"[FUNC_DEFS] Found functions: {list(func_defs.keys())}")
    ...
    for node in tree.body:
        ...
        if isinstance(node, ast.If):
            ...
            if cond_val is True:
                ...
                logger.debug("[MAIN_BLOCK] Processing __main__ block...")
                ...
                for func_name, arg_env in main_definite_calls:
                    ...
                    logger.debug(f"[MAIN_PROCESS] Added main call {func_name} to initial_calls")

    logger.debug(f"[INITIAL_CALLS] Initial calls collected: {initial_calls}")
    ...
    while stack:
        fn, argenv = stack.pop()
        logger.debug(f"[BFS_TRAVERSE] Processing function: {fn}, args: {argenv}")
    ...
    logger.debug(f"[ANALYSIS_SUMMARY] Reachable functions processed: {len(reachable)}")
    ...
    for fn_key in list(reachable):
        ...
        logger.debug(f"Function {fn_name} input calls: {len(definite_input_calls)}")
        ...
        for call_node in definite_input_calls:
            if not shadow_local:
                ...
                logger.debug("end static_analyze_code. Detected input call.")
        ...
        for wn in while_nodes:
            if isinstance(wn.test, ast.Constant) and wn.test.value is True:
                if not has_exit(wn):
                    ...
                    logger.debug("end static_analyze_code. Detected infinite while True loop without exit conditions.")
                    return CodeAnalyzeResult(
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            logger.debug(f"[MODULE_ASSIGN] Found assignment: {ast.unparse(node)}")
    ...
    logger.debug("end static_analyze_code. No timeout reason detected.")
    return None
