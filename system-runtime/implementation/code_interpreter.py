...
logger.info("code interpreter configuration loaded, ws_id=%s public_key_configured=%s", ws_id or "", bool(public_key))
...
async def run_code(
    ...
    raw_vm_envs = request.headers.get("x-tt-vm-envs", "")
    try:
        vm_envs = json.loads(raw_vm_envs) if raw_vm_envs else {}
        if not isinstance(vm_envs, dict):
            raise ValueError("x-tt-vm-envs must be a JSON object")
        vm_envs = {str(key): str(value) for key, value in vm_envs.items()}
    except (json.JSONDecodeError, ValueError) as e:
        logger.warning(f"ignore invalid x-tt-vm-envs: {e}")
        vm_envs = {}

    http_resp = await run_code_with_analyze_timeout(req, vm_envs, token)
    # 增加文件大小信息
...
async def run_code_with_analyze_timeout(
    ...
    try:
        ...
        logger.error(
            "code execution kernel initialization failed, kernel_id=%s generated_file_count=%d",
            req.kernel_id,
            len(ci_resp.result.path_list),
        )
        return ci_resp

    # 1 启动代码执行任务
    execute_task = asyncio.create_task(
        execute_code(req, kc, vm_envs)
    )
...
async def execute_code(
    ...
    logger.info(
        "code execution started, kernel_id=%s code_bytes=%d code_hash=%s timeout_seconds=%s",
        kernel_id,
    ...
    try:
        ...
        if vm_envs:
            env_code = f"import os as __mcp_os; __mcp_os.environ.update({vm_envs!r}); del __mcp_os"
            env_msg_id = kc.execute(env_code, silent=True, store_history=False)
            await _collect_messages(kc, env_msg_id, [], 5)
        ...
        try:
            ...
            ci_resp.result.path_list = get_generated_files(workspace, prev_files)
            logger.warning(
                "code execution timed out, kernel_id=%s output_item_count=%d generated_file_count=%d",
                kernel_id,
                len(contents),
                len(ci_resp.result.path_list),
            )
        ...
        ci_resp.result.path_list = get_generated_files(workspace, prev_files)
        logger.error(
            "code execution failed, kernel_id=%s output_item_count=%d generated_file_count=%d",
            kernel_id,
            len(contents),
            len(ci_resp.result.path_list),
        )
    ...
    logger.info(
        "code execution completed, kernel_id=%s success=%s output_item_count=%d generated_file_count=%d",
        kernel_id,
        ci_resp.result.success,
        len(contents),
        len(ci_resp.result.path_list),
    )
    return ci_resp
...
async def _collect_messages(kc: Any, msg_id: str, contents: list, timeout: int):
    ...
    while True:
        try:
            logger.debug("reading Jupyter IOPub message")
            ...
            logger.debug("Jupyter IOPub read retry, attempt=%d", retry_count)
            retry_count += 1
            time.sleep(0.01)
            continue
        message_type = iopub_msg["msg_type"]
        message_count += 1
        type_counts[message_type] = type_counts.get(message_type, 0) + 1
        logger.debug(
            "Jupyter IOPub message received, type=%s message_count=%d matches_request=%s",
            message_type,
            message_count,
            iopub_msg["parent_header"].get("msg_id") == msg_id,
        )

        if iopub_msg["parent_header"].get("msg_id") == msg_id:
            if message_type == "status":
                if iopub_msg["content"]["execution_state"] == "idle":
                    logger.debug("Jupyter IOPub execution state is idle")
                    break
                continue
    ...
    logger.debug("Jupyter IOPub summarized, message_count=%d type_counts=%s", message_count, type_counts)
