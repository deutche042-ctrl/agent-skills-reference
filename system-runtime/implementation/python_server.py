...
from application.trace_id_filter import set_agent_id, set_session_id, set_tool_use_id, set_trace_id
...
from application.computer_use import router as ComputerUseRouter
...
async def log_access_time(request: Request, call_next):
    ...
    set_agent_id(request.headers.get("x-agent-id", ""))
    set_trace_id(request.headers.get("x-tt-logid", ""))
    set_tool_use_id(request.headers.get("x-tool-use-id", ""))
    # check
    if request.url.path not in excluded_paths:
        # 获取当前时间
        last_access_times = time.time()
...
ci_app.include_router(ComputerUseRouter)
