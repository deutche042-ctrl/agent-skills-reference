...
skills_dir="${RUNTIME_INIT_SKILLS_DIR:-/runtime/skills}"
# 官方包本次解压产物的暂存目录（与真实 skills/ 同文件系统，便于 Go 侧 rename 搬运）。
# 真实目录的清空/搬运/差集删除统一由 Go 的 commitSkills 完成，本脚本不再直接改动 skills/。
tmp_official_dir="${RUNTIME_INIT_SKILLS_TMP_OFFICIAL_DIR:-/runtime/skills.tmp/official}"
...
# The Go Skills syncer owns all AgentWorkspaceRoot links. This script only
# consumes the official archive from the physical handoff mount.
if ! runtime_init_workspace_root_is_healthy; then
  runtime_init_log "workspace handoff root is not ready, skip load_session_skills"
  runtime_init_skipped "workspace handoff root is not ready"
fi
...
while [ "${attempt}" -le 2 ]; do
  runtime_init_log "download fallback skills archive attempt ${attempt}/2"
  rm -f "${tmp_file}"
  stage_start_ms="$(now_ms)"
