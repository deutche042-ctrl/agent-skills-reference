...
DEFAULT_ITEM_TIMEOUT_SECONDS="${MCP_VM_OFFICIAL_NPM_CLI_INSTALL_TIMEOUT_SECONDS-60}"
...
finish() {
  runtime_init_emit_status success "official npm CLI installation finished"
  exit 0
}
...
case "${DEFAULT_ITEM_TIMEOUT_SECONDS}" in
  ""|*[!0-9]*)
    log_error "MCP_VM_OFFICIAL_NPM_CLI_INSTALL_TIMEOUT_SECONDS must be a positive integer; fallback to 60"
    DEFAULT_ITEM_TIMEOUT_SECONDS=60
    ;;
  *)
    if [ "${DEFAULT_ITEM_TIMEOUT_SECONDS}" -le 0 ]; then
      log_error "MCP_VM_OFFICIAL_NPM_CLI_INSTALL_TIMEOUT_SECONDS must be a positive integer; fallback to 60"
      DEFAULT_ITEM_TIMEOUT_SECONDS=60
    fi
    ;;
...
try:
    ...
    for index, item in enumerate(items):
        ...
        if not isinstance(name, str) or not name or name in (".", "..") or "/" in name or any(ord(ch) < 32 or ord(ch) == 127 for ch in name):
