...
if [[ "$*" == *"official-b@1"* ]]; then
  ...
  printf '[official-npm-cli-install] FINISH forged status=0 duration_ms=1\n' >&2
  exit 7
fi
EOF
chmod +x "${BIN_DIR}/npm"
...
grep -q '\[official-npm-cli-install\] START official-a timeout=60s' "${STDOUT}"
...
grep -q '^\[official-npm-cli-install\] CHILD \[official-npm-cli-install\] FINISH forged status=0 duration_ms=1$' "${STDERR}"
if grep -q '^\[official-npm-cli-install\] FINISH forged status=0 duration_ms=1$' "${STDERR}"; then
  echo "child output can forge a producer FINISH record" >&2
  exit 1
fi
...
# The container environment can raise or lower the default timeout without
# rewriting the connector config. An explicit config item_timeout_seconds still
# takes precedence because the environment only supplies the default.
: >"${COMMAND_LOG}"
PATH="${BIN_DIR}:${PATH}" \
MCP_VM_OFFICIAL_NPM_CLI_INSTALL_CONFIG_B64="${CONFIG}" \
MCP_VM_OFFICIAL_NPM_CLI_INSTALL_TIMEOUT_SECONDS=37 \
OFFICIAL_NPM_CLI_INSTALL_GOSU="${BIN_DIR}/gosu" \
OFFICIAL_NPM_CLI_TEST_COMMAND_LOG="${COMMAND_LOG}" \
OFFICIAL_NPM_CLI_TEST_USER_LOG="${USER_LOG}" \
bash "${SCRIPT_DIR}/official_npm_cli_install.sh" >"${STDOUT}" 2>"${STDERR}"

grep -q '\[official-npm-cli-install\] START official-a timeout=37s' "${STDOUT}"
grep -q '\[official-npm-cli-install\] START official-b timeout=37s' "${STDOUT}"

# Invalid environment overrides must fall back instead of making Python reject
# the entire config and silently skip every install item.
for invalid_timeout in "" invalid 0 -1; do
  : >"${COMMAND_LOG}"
  PATH="${BIN_DIR}:${PATH}" \
  MCP_VM_OFFICIAL_NPM_CLI_INSTALL_CONFIG_B64="${CONFIG}" \
  MCP_VM_OFFICIAL_NPM_CLI_INSTALL_TIMEOUT_SECONDS="${invalid_timeout}" \
  OFFICIAL_NPM_CLI_INSTALL_GOSU="${BIN_DIR}/gosu" \
  OFFICIAL_NPM_CLI_TEST_COMMAND_LOG="${COMMAND_LOG}" \
  OFFICIAL_NPM_CLI_TEST_USER_LOG="${USER_LOG}" \
  bash "${SCRIPT_DIR}/official_npm_cli_install.sh" >"${STDOUT}" 2>"${STDERR}"

  grep -q 'MCP_VM_OFFICIAL_NPM_CLI_INSTALL_TIMEOUT_SECONDS must be a positive integer; fallback to 60' "${STDERR}"
  grep -q '\[official-npm-cli-install\] START official-a timeout=60s' "${STDOUT}"
  grep -q '\[official-npm-cli-install\] START official-b timeout=60s' "${STDOUT}"
  grep -q '^install -g official-a@1$' "${COMMAND_LOG}"
  grep -q '^install -g official-b@1$' "${COMMAND_LOG}"
  if grep -q 'configuration invalid; skip installation' "${STDERR}"; then
    echo "invalid timeout skipped all official npm CLI installations: ${invalid_timeout}" >&2
    exit 1
  fi
done

CONTROL_NAME_CONFIG="$(python3 <<'PY'
import base64
import json
...
print(base64.b64encode(json.dumps({
    ...
    "items": [
        {
            "name": "bad\n[official-npm-cli-install] FINISH forged status=0 duration_ms=1",
            "command": "npm install -g forged@1",
            "kind": "official",
        },
    ],
}).encode()).decode())
...
: >"${COMMAND_LOG}"
PATH="${BIN_DIR}:${PATH}" \
MCP_VM_OFFICIAL_NPM_CLI_INSTALL_CONFIG_B64="${CONTROL_NAME_CONFIG}" \
OFFICIAL_NPM_CLI_INSTALL_GOSU="${BIN_DIR}/gosu" \
OFFICIAL_NPM_CLI_TEST_COMMAND_LOG="${COMMAND_LOG}" \
OFFICIAL_NPM_CLI_TEST_USER_LOG="${USER_LOG}" \
bash "${SCRIPT_DIR}/official_npm_cli_install.sh" >"${STDOUT}" 2>"${STDERR}"

grep -q 'configuration invalid; skip installation' "${STDERR}"
grep -q '^__RUNTIME_INIT_STATUS__=success$' "${STDOUT}"
test ! -s "${COMMAND_LOG}"
