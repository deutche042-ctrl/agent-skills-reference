...
DEFAULT_SYNC_UNIT_TIMEOUT_SECONDS=5
DEFAULT_INOTIFY_COMPACT_MIN_LINES=10000
SYNC_PARALLELISM="${PERSISTENT_SYNC_PARALLELISM:-${DEFAULT_SYNC_PARALLELISM}}"
case "${SYNC_PARALLELISM}" in
  ''|*[!0-9]*) SYNC_PARALLELISM="${DEFAULT_SYNC_PARALLELISM}" ;;
  *)
    if [ "${SYNC_PARALLELISM}" -lt 1 ]; then
      SYNC_PARALLELISM="${DEFAULT_SYNC_PARALLELISM}"
    fi
    ;;
esac
SYNC_UNIT_TIMEOUT_SECONDS="${PERSISTENT_SYNC_UNIT_TIMEOUT_SECONDS:-${DEFAULT_SYNC_UNIT_TIMEOUT_SECONDS}}"
case "${SYNC_UNIT_TIMEOUT_SECONDS}" in
  ''|*[!0-9]*) SYNC_UNIT_TIMEOUT_SECONDS="${DEFAULT_SYNC_UNIT_TIMEOUT_SECONDS}" ;;
  *)
    if [ "${SYNC_UNIT_TIMEOUT_SECONDS}" -lt 1 ]; then
      SYNC_UNIT_TIMEOUT_SECONDS="${DEFAULT_SYNC_UNIT_TIMEOUT_SECONDS}"
    fi
    ;;
esac
...
# emit_status records a structured final result and, when requested by the Go
# runner, also emits its internal status marker. Every terminating path MUST
# call this exactly once.
emit_status() {
  local status="$1"
  case "${status}" in
    success) runtime_init_emit_status "${status}" "persistent ${MODE} completed" ;;
    skipped) runtime_init_emit_status "${status}" "persistent ${MODE} skipped" ;;
    *) runtime_init_emit_status "${status}" "persistent ${MODE} failed" ;;
  esac
}
...
def paths_overlap(left, right, sep=os.sep):
    return left == right or left.startswith(right + sep) or right.startswith(left + sep)


def unit_excludes_child(parent, child_path):
    parent_path = os.path.normpath(parent["local"])
    child_path = os.path.normpath(child_path)
    rel = os.path.relpath(child_path, parent_path).replace(os.sep, "/")
    if rel == "." or rel.startswith("../"):
        return False
    for exclude in parent["excludes"]:
        if not isinstance(exclude, str):
            continue
        normalized = exclude.replace("\\", "/").lstrip("/")
        if normalized == rel or normalized == rel + "/":
            return True
    return False


def excluded_nested_overlap(left, right):
    left_path = os.path.normpath(left["local"])
    right_path = os.path.normpath(right["local"])
    if left_path == right_path:
        return False
    if left_path.startswith(right_path + os.sep):
        return unit_excludes_child(right, left_path)
    if right_path.startswith(left_path + os.sep):
        return unit_excludes_child(left, right_path)
    return False
...
for unit in config.get("units", []):
    ...
    if name in names:
        raise ValueError("unit name must be unique")
    names.add(name)
    ...
    for previous in units:
        if paths_overlap(current["local"], previous["local"]) and not excluded_nested_overlap(current, previous):
            raise ValueError(f"units {name!r} and {previous['name']!r} have overlapping local paths")
        if paths_overlap(current["remote"], previous["remote"], "/"):
            raise ValueError(f"units {name!r} and {previous['name']!r} have overlapping remote paths")
...
run_rsync_capture_with_timeout() {
  local direction="$1" name="$2" output_file="$3"
  shift 3
  local start_ms pid elapsed_ms timeout_ms status
  start_ms="$(now_ms)"
  timeout_ms=$((SYNC_UNIT_TIMEOUT_SECONDS * 1000))
  : >"${output_file}"
  "$@" >"${output_file}" 2>&1 &
  pid=$!
  while kill -0 "${pid}" 2>/dev/null; do
    ...
    if [ "${elapsed_ms}" -ge "${timeout_ms}" ]; then
      log "sync timeout: direction=${direction} unit=${name} timeout_seconds=${SYNC_UNIT_TIMEOUT_SECONDS} elapsed_ms=${elapsed_ms}"
...
sync_one_file() {
  ...
  if [ "${direction}" = "restore" ] || [ "${direction}" = "pull" ]; then
    local rsync_output rsync_output_file rsync_status
    ...
    rsync_output_file="$(mktemp "${WORK_DIR}/rsync-output.XXXXXX")"
    if run_rsync_capture_with_timeout "${direction}" "${name}" "${rsync_output_file}" rsync -a --itemize-changes --chown="${SYNC_OWNER}" "--exclude-from=${excludes_file}" "${remote_file}" "${local}"; then
      ...
      rsync_status=$?
      rsync_output="$(cat "${rsync_output_file}" 2>/dev/null || true)"
      rm -f "${rsync_output_file}"
      if [ "${rsync_status}" -eq 124 ]; then
        log_sync_finish "${direction}" "${name}" "timeout" "${start_ms}" true "${use_delete}"
        return 1
      fi
    ...
    rsync_output_file="$(mktemp "${WORK_DIR}/rsync-output.XXXXXX")"
    if run_rsync_capture_with_timeout "push" "${name}" "${rsync_output_file}" rsync -a --itemize-changes --chown=root:root "--exclude-from=${excludes_file}" "${local}" "${remote_file}"; then
      rsync_output="$(cat "${rsync_output_file}")"
      rm -f "${rsync_output_file}"
      ...
      rm -f "${rsync_output_file}"
      if [ "${rsync_status}" -eq 124 ]; then
        log_sync_finish "push" "${name}" "timeout" "${start_ms}" true "${use_delete}"
        return 1
      fi
...
sync_one() {
  ...
  if [ "${direction}" = "restore" ] || [ "${direction}" = "pull" ]; then
    local rsync_output rsync_output_file rsync_status
    rsync_output_file="$(mktemp "${WORK_DIR}/rsync-output.XXXXXX")"
    if run_rsync_capture_with_timeout "${direction}" "${name}" "${rsync_output_file}" rsync -a --itemize-changes --chown="${SYNC_OWNER}" "--exclude-from=${excludes_file}" "${remote_dir}/" "${local}/"; then
      rsync_output="$(cat "${rsync_output_file}")"
      rm -f "${rsync_output_file}"
      log_rsync_changes "${direction}" "${name}" "${rsync_output}"
      log "${direction} succeeded: ${name}"
      ...
      rm -f "${rsync_output_file}"
      if [ "${rsync_status}" -eq 124 ]; then
        log_sync_finish "${direction}" "${name}" "timeout" "${start_ms}" false "${use_delete}"
        return 1
      fi
      log "${direction} failed: ${name}" >&2
      [ -n "${rsync_output}" ] && printf '%s\n' "${rsync_output}" >&2
    ...
    local rsync_output rsync_output_file rsync_status
    rsync_output_file="$(mktemp "${WORK_DIR}/rsync-output.XXXXXX")"
    if run_rsync_capture_with_timeout "push" "${name}" "${rsync_output_file}" rsync -a --itemize-changes --chown=root:root "--exclude-from=${excludes_file}" "${local}/" "${remote_dir}/"; then
      rsync_output="$(cat "${rsync_output_file}")"
      rm -f "${rsync_output_file}"
      log_rsync_changes "push" "${name}" "${rsync_output}"
      log "push succeeded: ${name}"
      ...
      rm -f "${rsync_output_file}"
      if [ "${rsync_status}" -eq 124 ]; then
        log_sync_finish "push" "${name}" "timeout" "${start_ms}" false "${use_delete}"
        return 1
      fi
      log "push failed: ${name}" >&2
      [ -n "${rsync_output}" ] && printf '%s\n' "${rsync_output}" >&2
...
sync_units() {
  ...
  local had_error=0 running_pids="" running_status_files="" running_count=0 unit_index=0 status_dir
  local sync_start_ms unit_count=0 selected_units="" sync_status
  local name root remote local push_delete_enabled excludes is_file pid status_file first_pid first_status_file unit_status
  if [ ! -s "${UNITS_FILE}" ]; then
    return 0
  fi
  sync_start_ms="$(now_ms)"
  log "sync units start: direction=${direction} target_unit=${target_unit:-all} parallelism=${SYNC_PARALLELISM}"
  status_dir="${WORK_DIR}/sync-status.$$"
  rm -rf "${status_dir}"
  mkdir -p "${status_dir}"
  while IFS=$'\t' read -r name root remote local push_delete_enabled excludes is_file; do
    ...
    selected_units="${selected_units}${selected_units:+,}${name}"
    status_file="${status_dir}/unit.${unit_index}.status"
    (
      set +e
      sync_one "${direction}" "${name}" "${root}" "${remote}" "${local}" "${push_delete_enabled}" "${excludes}" "${is_file}" "${use_delete}"
      printf '%s' "$?" >"${status_file}"
      exit 0
    ) &
    pid=$!
    running_pids="${running_pids}${running_pids:+ }${pid}"
    running_status_files="${running_status_files}${running_status_files:+ }${status_file}"
    running_count=$((running_count + 1))
    if [ "${running_count}" -ge "${SYNC_PARALLELISM}" ]; then
      first_pid="${running_pids%% *}"
      first_status_file="${running_status_files%% *}"
      if [ "${running_pids}" = "${first_pid}" ]; then
        running_pids=""
      else
        running_pids="${running_pids#* }"
      fi
      if [ "${running_status_files}" = "${first_status_file}" ]; then
        running_status_files=""
      else
        running_status_files="${running_status_files#* }"
      fi
      wait "${first_pid}" 2>/dev/null || true
      unit_status="$(cat "${first_status_file}" 2>/dev/null || echo 1)"
      [ "${unit_status}" -eq 0 ] || had_error=1
      running_count=$((running_count - 1))
    fi
  done <"${UNITS_FILE}"
  ...
  log "sync units finish: direction=${direction} target_unit=${target_unit:-all} status=${sync_status} duration_ms=$(( $(now_ms) - sync_start_ms )) units=${unit_count} parallelism=${SYNC_PARALLELISM} unit_names=${selected_units:-none}"
  return "${had_error}"
}
