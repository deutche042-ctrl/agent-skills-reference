...
if [ -n "${PERSISTENT_SYNC_TEST_RSYNC_SLEEP_SOURCE:-}" ]; then
  case "$*" in
    *"${PERSISTENT_SYNC_TEST_RSYNC_SLEEP_SOURCE}"*)
      /bin/sleep "${PERSISTENT_SYNC_TEST_RSYNC_SLEEP_SOURCE_SECONDS}"
      ;;
  esac
fi
...
test_restore_syncs_independent_units_in_parallel() {
  local root cfg out start_ms end_ms elapsed_ms rsync_count
  root="$(make_case)"
  cfg="$(encode_config "${root}/persistent" "${root}/home" 'cfg["units"] = [{"name": "unit-a", "local_path": local_root + "/.a", "remote_path": "unit-a"}, {"name": "unit-b", "local_path": local_root + "/.b", "remote_path": "unit-b"}, {"name": "unit-c", "local_path": local_root + "/.c", "remote_path": "unit-c"}]')"
  mkdir -p "${root}/persistent/unit-a" "${root}/persistent/unit-b" "${root}/persistent/unit-c"
  start_ms="$(now_ms_for_test)"
  out="$(run_script "${root}" "${cfg}" restore PERSISTENT_SYNC_TEST_RSYNC_SLEEP_SECONDS=1)"
  end_ms="$(now_ms_for_test)"
  elapsed_ms=$(( end_ms - start_ms ))
  if [ -f "${root}/rsync.log" ]; then
    rsync_count="$(wc -l < "${root}/rsync.log" | tr -d ' ')"
  else
    rsync_count=0
  fi
  # Three one-second jobs must overlap; allow room for the final structured
  # runtime-init log on slower local shells.
  if [ "$(rc_of "${root}")" -eq 0 ] \
     && echo "${out}" | grep -q '__RUNTIME_INIT_STATUS__=success' \
     && [ "${rsync_count}" -eq 3 ] \
     && [ "${elapsed_ms}" -lt 3000 ]; then
    pass "restore syncs independent units in parallel"
  else
    printf 'parallel restore diagnostics: elapsed_ms=%s rsync_count=%s out=%s\n' "${elapsed_ms}" "${rsync_count}" "${out}" >&2
    fail "restore should sync independent units in parallel"
  fi
  rm -rf "${root}"
}

test_sync_units_logs_parallel_batch_summary() {
  local root cfg out
  root="$(make_case)"
  cfg="$(encode_config "${root}/persistent" "${root}/home" 'cfg["units"] = [{"name": "unit-a", "local_path": local_root + "/.a", "remote_path": "unit-a"}, {"name": "unit-b", "local_path": local_root + "/.b", "remote_path": "unit-b"}]')"
  out="$(run_script "${root}" "${cfg}" restore PERSISTENT_SYNC_PARALLELISM=2)"
  if echo "${out}" | grep -q '\[persistent-sync\] sync units start: direction=restore target_unit=all parallelism=2' \
     && echo "${out}" | grep -Eq '\[persistent-sync\] sync units finish: direction=restore target_unit=all status=success duration_ms=[0-9]+ units=2 parallelism=2 unit_names=unit-a,unit-b'; then
    pass "sync units logs parallel batch summary"
  else
    fail "sync units should log parallel batch summary (out: ${out})"
  fi
  rm -rf "${root}"
}

test_timed_out_unit_releases_slot_for_later_units() {
  local root cfg out
  root="$(make_case)"
  cfg="$(encode_config "${root}/persistent" "${root}/home" 'cfg["units"] = [{"name": "slow-unit", "local_path": local_root + "/.slow", "remote_path": "slow-unit"}, {"name": "fast-unit", "local_path": local_root + "/.fast", "remote_path": "fast-unit"}]')"
  mkdir -p "${root}/persistent/slow-unit" "${root}/persistent/fast-unit"
  out="$(run_script "${root}" "${cfg}" restore \
    PERSISTENT_SYNC_PARALLELISM=1 \
    PERSISTENT_SYNC_UNIT_TIMEOUT_SECONDS=1 \
    PERSISTENT_SYNC_TEST_RSYNC_SLEEP_SOURCE="${root}/persistent/slow-unit/" \
    PERSISTENT_SYNC_TEST_RSYNC_SLEEP_SOURCE_SECONDS=2)"
  sleep 2
  if echo "${out}" | grep -q 'sync timeout: direction=restore unit=slow-unit timeout_seconds=1' \
     && echo "${out}" | grep -q 'sync finish: direction=restore unit=slow-unit status=timeout' \
     && echo "${out}" | grep -q 'sync finish: direction=restore unit=fast-unit status=success' \
     && echo "${out}" | grep -q 'sync units finish: direction=restore target_unit=all status=partial_failed' \
     && echo "${out}" | grep -q 'restore timing: status=partial_failed' \
     && grep -q -- "${root}/persistent/fast-unit/" "${root}/rsync.log" 2>/dev/null \
     && ! grep -q -- "${root}/persistent/slow-unit/" "${root}/rsync.log" 2>/dev/null; then
    pass "timed out unit releases slot for later units"
  else
    fail "timed out unit should release slot for later units without late rsync side effects (out: ${out}; rsync: $(cat "${root}/rsync.log" 2>/dev/null))"
  fi
  rm -rf "${root}"
}

test_invalid_overlapping_local_without_parent_exclude_fails() {
  local root cfg out
  root="$(make_case)"
  cfg="$(encode_config "${root}/persistent" "${root}/home" 'cfg["units"] = [{"name": "home-config", "local_path": local_root + "/.config", "remote_path": "home-config", "excludes": []}, {"name": "wecom-config", "local_path": local_root + "/.config/wecom", "remote_path": "wecom-config"}]')"
  out="$(run_script "${root}" "${cfg}" restore)"
  if [ "$(rc_of "${root}")" -ne 0 ] \
     && echo "${out}" | grep -q '__RUNTIME_INIT_STATUS__=failed' \
     && echo "${out}" | grep -q 'overlapping local paths'; then
    pass "overlapping local paths without parent exclude hard-fail"
  else
    printf 'overlapping local diagnostics: rc=%s out=%s\n' "$(rc_of "${root}")" "${out}" >&2
    fail "overlapping local paths without parent exclude should hard-fail"
  fi
  rm -rf "${root}"
}

test_invalid_overlapping_remote_paths_fail() {
  local root cfg out
  root="$(make_case)"
  cfg="$(encode_config "${root}/persistent" "${root}/home" 'cfg["units"] = [{"name": "parent-remote", "local_path": local_root + "/.parent-remote", "remote_path": "shared"}, {"name": "child-remote", "local_path": local_root + "/.child-remote", "remote_path": "shared/child"}]')"
  out="$(run_script "${root}" "${cfg}" restore)"
  if [ "$(rc_of "${root}")" -ne 0 ] \
     && echo "${out}" | grep -q '__RUNTIME_INIT_STATUS__=failed' \
     && echo "${out}" | grep -q 'overlapping remote paths'; then
    pass "overlapping remote paths hard-fail"
  else
    printf 'overlapping remote diagnostics: rc=%s out=%s\n' "$(rc_of "${root}")" "${out}" >&2
    fail "overlapping remote paths should hard-fail"
  fi
  rm -rf "${root}"
}
