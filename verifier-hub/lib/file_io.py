...
def cmd_validate(args: argparse.Namespace) -> dict:
    ...
    try:
        if kind == "xlsx":
            openpyxl = C.lazy_import("openpyxl", hint="pip install openpyxl")
            wb = openpyxl.load_workbook(path, read_only=True, data_only=True)
            detail["sheets"] = list(wb.sheetnames)
            wb.close()
        elif kind == "docx":
            docx = C.lazy_import("docx", hint="pip install python-docx")
            ...
            detail["page_count"] = len(reader.pages)
        elif kind == "pptx":
            pptx = C.lazy_import("pptx", hint="pip install python-pptx")
